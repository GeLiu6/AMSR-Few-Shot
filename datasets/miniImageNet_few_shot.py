...
class SimpleDataset_for_selection:
    def __init__(self, transform, target_transform=identity):
        self.transform = transform
        self.target_transform = target_transform
    ...
    def __getitem__(self, i):
        ...
        # return self.meta['image_names'][i], img, target
        return np.array(image), img, target

    def __len__(self):
        # return len(self.meta['image_names'])
        return len(self.d)

class proxy_image_dataset:
    def __init__(self, transform, target_transform=identity):
        self.transform = transform
        self.target_transform = target_transform
        ...
        from PIL import Image
        for i in range(64):
            image = Image.open('./proxy_images/%d.jpeg'%(i)).convert('RGB')
    ...
    def __getitem__(self, i):
        ...
        target = self.target_transform(self.labels[i])
...
class ProxyImageManager(DataManager):
    def __init__(self, image_size, batch_size):        
        super(ProxyImageManager, self).__init__()
        self.batch_size = batch_size
        self.trans_loader = TransformLoader(image_size)

    def get_data_loader(self, aug): #parameters that would change on train/val set
        transform = self.trans_loader.get_composed_transform(aug)
        dataset = proxy_image_dataset(transform)
...
class SimpleDataManager_for_selection(DataManager):
    def __init__(self, image_size, batch_size):        
        super(SimpleDataManager_for_selection, self).__init__()
        self.batch_size = batch_size
        self.trans_loader = TransformLoader(image_size)
    ...
    def get_data_loader(self, aug): #parameters that would change on train/val set
        ...
        transform = self.trans_loader.get_composed_transform(aug)
        dataset = SimpleDataset_for_selection(transform)
